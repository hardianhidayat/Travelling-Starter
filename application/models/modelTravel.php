...
class ModelTravel extends CI_Model{
    ...
    // guide model
    public function ambilsemuaguide(){
        return $this->db->get('guide');
    }

    public function ambilsemuaguidelimit($start, $end){
        $sql = "SELECT * FROM guide ORDER BY id DESC LIMIT $start, $end";
        $query = $this->db->query($sql);        
        return $query;
    }

    public function ambilGuideId($id){
    ...
    public function ambildatatravel($no){
        ...
        $sql = "SELECT destinasi.id, nama_destinasi, destinasi.gambar, loc, id_pulau, harga, pulau.banner FROM destinasi JOIN pulau ON id_pulau = pulau.id ORDER BY id_pulau DESC LIMIT $no,6";
        $query = $this->db->query($sql);        
        return $query;
    }
    ...
    public function ambilsemuatravellimit($start, $end){
        $sql = "SELECT destinasi.id, nama_destinasi, destinasi.gambar, loc, id_pulau, harga, pulau.banner FROM destinasi JOIN pulau ON id_pulau = pulau.id ORDER BY id_pulau DESC LIMIT $start, $end";
        $query = $this->db->query($sql);        
        return $query;
    }
    ...
    public function ambilsemuainvoice($start, $end){
        $sql = "SELECT inv.*, u.nama AS nama_user, nama_destinasi
        FROM invoice inv 
