...
class Admin extends CI_Controller
{
    public function index()
    {
        if ($this->session->admin) {
            ...
            echo "<script> window.location.href='/traveller/admin/login'; </script>";
        }
    }
    ...
    public function login()
    {
        if (!$this->session->admin)
            ...
            echo "<script> window.location.href='/traveller/admin'; </script>";
        }
    }
    ...
    public function tuklogin()
    {
        ...
        if ($email === $admin['user'] && $password === $admin['password']) {
            ...
            echo "<script> alert('Password salah bre');
                            window.location.href='/traveller/admin/login';
                  </script>";
        }
    }
    ...
    public function hapuslokasi($id)
    {
        ...
        echo "<script>window.location.href='/traveller/admin/lokasi';</script>";
    }
    ...
    public function hapusdestinasi($id)
    {
        ...
        echo "<script>window.location.href='/traveller/admin/destinasi';</script>";
    }
    ...
    {
        $config['base_url'] = base_url('admin/guide/');
        $config['total_rows'] = $this->modelTravel->ambilsemuaguide()->num_rows();
        $config['per_page'] = 5;
        $this->pagination->initialize($config);
        $data['start'] = $this->uri->segment(3);
        if (!$data['start'])
        ...
        $data['guide'] = $this->modelTravel->ambilsemuaguidelimit($data['start'], $config['per_page'])->result_array();
        $this->load->view('admin/templates/header');
    ...
    {
        $data['guide'] = $this->modelTravel->ambilsemuaguide()->result_array();
        $this->load->view('admin/templates/header');
        $this->load->view('admin/tambahguide', $data);
        $this->load->view('admin/templates/footer');
    }
    ...
    {
        $data['guide'] = $this->modelTravel->ambilGuideId($id)->row_array();
        $this->load->view('admin/templates/header');
        $this->load->view('admin/editguide', $data);
        $this->load->view('admin/templates/footer');
    }

    public function hapusguide($id)
    {
        $data = $this->modelTravel->ambilGuideId($id)->row_array();
        $path = FCPATH . "assets\img\place\\" . $data['gambar'];
        unlink($path);
        $this->db->delete('guide', ['id' => $id]);
        echo "<script>window.location.href='/traveller/admin/guide';</script>";
    }
